...
def gt_id_gen():
    ...
    for i in root.findall(".//s"):
        annotations.append(
            [int(float(i.get("i"))), int(float(i.get("x"))), int(float(i.get("y")))]
        )
    j = 0
...
def matching():
    ...
    id_lis[:, 0] = gts[:, 0]
    df = pd.DataFrame(
        data=id_lis[:, 1:5], columns=["expert1", "expert2", "expert3", "expert4"]
    )
...
def associate(plot_size, gts, expert, df):
    res_paths = sorted(
        Path(
            "/home/kazuya/ssd/detection/output/test18/MSELoss/%s/res" % plot_size
        ).glob("*.tif")
    )
